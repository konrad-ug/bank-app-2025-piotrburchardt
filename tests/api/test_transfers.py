...
def test_incoming_transfer_success(client):
    ...
    response = client.post(
        f"{BASE_URL}/api/accounts/{payload['pesel']}/transfer",
        json={"amount": 500, "type": "incoming"},
    )

    assert response.status_code == 200
    assert response.json()["message"] == "Zlecenie przyjęto do realizacji"
    account_data = client.get(f"{BASE_URL}/api/accounts/{payload['pesel']}").json()
    assert account_data["balance"] == 500
...
def test_express_transfer_success(client):
    ...
    assert response.json()["message"] == "Zlecenie przyjęto do realizacji"
    account_data = client.get(f"{BASE_URL}/api/accounts/{payload['pesel']}").json()
    assert account_data["balance"] == 49
